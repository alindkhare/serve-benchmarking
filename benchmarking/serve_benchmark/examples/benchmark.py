...
def main(batch_size, num_warmups, num_queries, return_type):
    ...
    def noop(_, *args, **kwargs):
        bs = serve_benchmark.context.batch_size
        assert (
            bs == batch_size
    ...
    if batch_size:
        noop = serve_benchmark.accept_batch(noop)

    with serve_benchmark.using_router("noop"):
        serve_benchmark.create_endpoint("noop", "/noop")
        config = serve_benchmark.BackendConfig(max_batch_size=batch_size)
        serve_benchmark.create_backend(noop, "noop", backend_config=config)
        serve_benchmark.link("noop", "noop")
        handle = serve_benchmark.get_handle("noop")
