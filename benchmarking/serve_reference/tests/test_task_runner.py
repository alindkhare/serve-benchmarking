...
import benchmarking.serve_reference.context as context
from benchmarking.serve_reference.policy import RoundRobinPolicyQueueActor
...
async def test_task_runner_custom_method_batch(serve_instance):
    ...
    class Batcher:
        def a(self, _):
            return [
                "a-{}".format(i)
                for i in range(serve_reference.context.batch_size)
            ]
