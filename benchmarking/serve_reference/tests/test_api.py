...
from benchmarking.serve_reference import BackendConfig
import ray
from benchmarking.serve_reference.constants import NO_ROUTE_KEY
from benchmarking.serve_reference.exceptions import RayServeException
...
def test_e2e(serve_instance):
    ...
    serve_reference.create_endpoint("endpoint", "/api", methods=["GET", "POST"])
    ...
    while True:
        try:
            resp = requests.get(
                "http://127.0.0.1:8000/-/routes", timeout=0.5
            ).json()
            ...
            if retry_count == 0:
                assert False, (
                    "Route table hasn't been updated after 3 tries."
...
def test_route_decorator(serve_instance):
    @serve_reference.route("/hello_world")
    def hello_world(_):
        return ""
    ...
    hello_world.scale(2)
    assert (
        serve_reference.get_backend_config("hello_world:v0").num_replicas == 2
    )
...
def test_killing_replicas(serve_instance):
    ...
    serve_reference.create_backend(Simple, "simple:v1", backend_config=b_config)
    global_state = serve_reference.api._get_global_state()
