...
import ray
from benchmarking.serve_reference import context as serve_context
from benchmarking.serve_reference.context import FakeFlaskRequest
from collections import defaultdict
from benchmarking.serve_reference.utils import (
    parse_request_item,
    ray_pin_to_core,
    tracer,
    from_batchable_kwargs,
)
...
class TaskRunner:
    ...
    That is, a ray serve_reference actor should implement the TaskRunner interface.
    """
...
class RayServeMixin:
    ...
    Warning:
        ...
        class. This means that serve_reference will call `your_instance.__call__` when
    ...
    def invoke_batch(self, request_item_list):
        ...
        # have same serve_reference context.
    ...
    def _ray_serve_call_ref(self, metadata, **kwargs):
        ...
        if metadata["batch_size"]:
            kwargs_list = from_batchable_kwargs(
...
class TaskRunnerBackend(TaskRunner, RayServeMixin):
    ...
    Note:
    This class is not used in the actual ray serve_reference system. It exists
