...
import ray
from benchmarking.serve_reference.constants import (
    DEFAULT_HTTP_HOST,
    DEFAULT_HTTP_PORT,
    SERVE_MASTER_NAME,
)
from benchmarking.serve_reference.global_state import (
    GlobalState,
    start_initial_state,
)
from benchmarking.serve_reference.kv_store_service import SQLiteKVStore
from benchmarking.serve_reference.task_runner import (
    RayServeMixin,
    TaskRunnerActor,
)
from benchmarking.serve_reference.utils import get_random_letters, expand
from benchmarking.serve_reference.exceptions import (
    RayServeException,
...
from benchmarking.serve_reference.queues import Query
from benchmarking.serve_reference.request_params import RequestMetadata

global_state = None


def _get_global_state():
    """Used for internal purpose. Because just import benchmarking.serve_reference.global_state
    ...
    def check(*args, **kwargs):
        if _get_global_state() is None:
            raise RayServeException(
                "Please run serve_reference.init to initialize or "
                "connect to existing ray serve_reference cluster."
            )
...
def init(
    ...
    """Initialize a serve_reference cluster.

    If serve_reference cluster has already initialized, this function will just return.

    Calling `ray.init` before `serve_reference.init` is optional. When there is not a ray
    cluster initialized, serve_reference will call `ray.init` with `object_store_memory`
    requirement.
    ...
    Args:
        ...
        start_server (bool): If true, `serve_reference.init` starts http server.
            (Default: True)
    ...
    assert (
        not start_server
    ), "serve_reference assume no HTTP started in serve_reference"
    ...
    # Try to get serve_reference master actor if it exists
    try:
...
def link(endpoint_name, backend_tag):
    ...
    >>> serve_reference.link("service-name", "backend:v1")

    Note:
    This is equivalent to

    >>> serve_reference.split("service-name", {"backend:v1": 1.0})
    """
...
def split(endpoint_name, traffic_policy_dictionary):
    ...
    >>> serve_reference.split("service-name", {
        "backend:v1": 0.5,
        "backend:v2": 0.5
    })
...
def get_handle(
    ...
    from benchmarking.serve_reference.handle import RayServeHandle
