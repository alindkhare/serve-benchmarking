...
from benchmarking.serve_reference.kv_store_service import (
    BackendTable,
    RoutingTable,
    TrafficPolicyTable,
)
...
from benchmarking.serve_reference.http_proxy import HTTPProxyActor
from benchmarking.serve_reference.utils import ray_pin_to_core
...
class ServeMaster:
    ...
    def start_actor(
        self, actor_cls, tag, init_args=(), init_kwargs={}, is_asyncio=False
    ):
...
class GlobalState:
    ...
    def __init__(self, master_actor_handle=None, start_server=True):
    ...
    def init_or_get_router(
        ...
        assert self.router_name != "default", (
            ...
            "`with serve_reference.using_router(name):` to assign router "
            "per endpiont?"
        )

        queue_actor_tag = f"queue::{self.router_name}::" + queueing_policy.name
        if queue_actor_tag not in self.actor_handle_cache:
            [handle] = ray.get(
                self.master_actor_handle.start_actor.remote(
                    queueing_policy.value,
