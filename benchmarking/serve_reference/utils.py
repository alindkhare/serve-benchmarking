...
from benchmarking.serve_reference.context import FakeFlaskRequest, TaskContext
from benchmarking.serve_reference.http_util import build_flask_request
import itertools
import numpy as np
import ray
...
def to_batchable_kwargs(requests):
    new_kwargs = dict()
    for batch_idx, request in enumerate(requests):
        for kwarg in request.request_kwargs.keys():
            new_kwargs[kwarg + "_" + str(batch_idx)] = request.request_kwargs[
                kwarg
            ]
    return new_kwargs


def from_batchable_kwargs(kwarg_unwrapped, kwarg_keys, batch_size):
    kwarg_list = defaultdict(list)
    for batch_idx in range(batch_size):
        for kwarg in kwarg_keys:
            kwarg_list[kwarg].append(
                kwarg_unwrapped[kwarg + "_" + str(batch_idx)]
            )
    return kwarg_list
...
def expand(l):
    ...
    Example:
    >>> serve_reference.utils.expand([1,2,[3,4,5],6])
    [1,2,3,4,5,6]
    >>> serve_reference.utils.expand(["a", ["b", "c"], "d", ["e", "f"]])
    ["a", "b", "c", "d", "e", "f"]
    """
    return list(
...
def _get_logger():
    logger = logging.getLogger("serve_reference")
